...
시드 DB 약 검색 → P1 처방 입력 → 발급(토큰+실제 SVG QR) → 환자 폰 스캔 → 하루 흐름 복약 포스터 열람.
...
cd server && uv run pytest -q                             # 테스트 (38 passed)
```
...
| 상태 | 항목 |
|---|---|
| **구현** | 발급 API(멱등·검증 422·오프라인 토큰 409)·reissue(D5)·drugs 검색·이벤트 비콘·S1 하루 시간 흐름 포스터(첫눈 실약명·봉투 색+번호·단위별 자체 SVG·PRN 1회량/일일최대/최소간격·식전후 순서·기간·hi/en D12)·대기/410/코드 재입력/개인정보 화면·view.first 원자 선점·ivid 쿠키·P1 자동완성/확인·P3 실제 SVG QR/인쇄/전체화면·3종 데모·테스트 38건 |
