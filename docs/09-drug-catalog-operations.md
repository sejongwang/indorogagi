...
같은 package라도 importer 또는 normalization version이 바뀌면 별도 import run으로 처리된다. importer는 검색·표시·성분·별칭·포장까지 포함한 deterministic normalized projection JSON과 SHA-256을 이전 projection과 비교하고, changed field diff와 review 필요 여부를 run-record와 JSON/Markdown 보고서에 남긴다. projection이 같으면 source evidence `record_version`은 증가해 오래된 retirement 후보를 stale로 만들지만 기존 `approved/rejected` 결정은 보존한다. projection이 달라졌을 때만 기존 `approved`를 `needs_review`로 되돌린다. 이는 변경 결과가 자동 승인된다는 뜻이 아니다. 사람의 `rejected`는 projection이 달라져도 유지한다 — `needs_review`는 검색 가능 상태라 자동 리셋은 사람의 검색 차단을 해제하며(§7 excluded 재등장 처리와 동일 원칙), 재검토 개시는 사람의 `review_requested`로만 한다. 성공한 write 뒤에만 presentation의 `current_import_run_record_id`를 새 run-record로 옮긴다. 동일 raw를 재사용한 후속 실행이 persistence 단계에서 실패하면 실패 run-record는 최신 evidence로 보이지만, 사람 결정과 처방 provenance는 계속 마지막 적용 run을 참조한다. `code_revision`은 실행 commit SHA를 기록할 수 있는 선택 필드이며, 비어 있어도 semantic processing versions와 결정 당시 normalized snapshot은 필수다.
